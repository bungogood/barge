...
async fn main() -> anyhow::Result<()> {
    ...
    let test = warp::path("ping")
        .and(warp::post())
        .and(barge_filter.clone())
        .and_then(|_barge: Arc<Barge>| async move {
            let resp = Success {
                data: "ping".to_string(),
            };
            let json = warp::reply::json(&resp);
    ...
    let pong = warp::path("pong")
        ...
        .and_then(|_barge: Arc<Barge>| async move {
            ...
            let json = warp::reply::json(&prob);
            let reply = warp::reply::with_status(json, StatusCode::CONFLICT);
            let reply = warp::reply::with_header(reply, "x-leader", leader_hint);
            Ok::<_, warp::Rejection>(reply.into_response())
        });

    let routes = propose.or(test).or(pong);
